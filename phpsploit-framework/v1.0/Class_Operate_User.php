...
class Class_Operate_User extends Class_Operate implements Interface_Operate_User
{
    public static function exist_token ()
    {
        if ( Class_Base_Extension::exist_enabled_extensions ( Class_Base_Extension::EXTENSION_NAME_SHMOP ) ) {
            if(!is_cli()){
                $_key   = Class_Base_Auth::SHM_WEB_KEY;
            }else{
                $_key   = Class_Base_Auth::SHM_CLI_KEY;
            }
            ...
            if ( ! empty( $_shmid ) ) {
                $_share_memory_security_token = Class_Base_Memory::read_share_memory ( $_shmid , Class_Base_Memory::SHARE_MEMORY_OFFSET_START , Class_Base_Memory::BLOCK_SIZE_32 , Class_Base_Memory::DATA_FORMAT_TYPE_STRING_NULL_FILL_PACK );
                if ( ! empty( $_share_memory_security_token ) ) {
                    $_share_memory_security_token = Class_Base_Format::data_to_string ( $_share_memory_security_token );
                    if ( ! empty( $_share_memory_security_token ) ) {
                        return true;
                    }
                }
    ...
    public static function reset_token ( $password = null )
    {
        if ( Class_Base_Extension::exist_enabled_extensions ( Class_Base_Extension::EXTENSION_NAME_SHMOP ) ) {
            ...
            if ( ! empty( $_shmid ) ) {
                ...
                $_security_token              = md5 ( $_password . $_security_code );
                $_share_memory_security_token = md5 ( $_security_token . $_security_code );
                $_write_length                = Class_Base_Memory::write_share_memory ( $_shmid , $_share_memory_security_token , Class_Base_Memory::SHARE_MEMORY_OFFSET_START , Class_Base_Memory::DATA_FORMAT_TYPE_STRING_NULL_FILL_PACK );
            ...
            $_security_token = md5 ( $password . Class_Base_Auth::SECURITY_CODE );
            return $_security_token;
        }
        return null;
    }
    ...
    public static function create_token ( &$password = null )
    {
        if ( Class_Base_Extension::exist_enabled_extensions ( Class_Base_Extension::EXTENSION_NAME_SHMOP ) ) {
            ...
            if ( empty( $_shmid ) ) {
                ...
                if ( ! empty( $_shmid ) ) {
                    $password                     = Class_Base_Auth::create_password ();
                    $_security_token              = md5 ( $password . $_security_code );
                    $_share_memory_security_token = md5 ( $_security_token . $_security_code );
                    $_write_length                = Class_Base_Memory::write_share_memory ( $_shmid , $_share_memory_security_token , Class_Base_Memory::SHARE_MEMORY_OFFSET_START , Class_Base_Memory::DATA_FORMAT_TYPE_STRING_NULL_FILL_PACK );
                    if ( empty( $_write_length ) ) {
                        Class_Base_Memory::delete_share_memory_by_key ( $_key , Class_Base_Memory::BLOCK_SIZE_32 );
    ...
    public static function check_security_token ( $security_token )
    {
        if ( Class_Base_Extension::exist_enabled_extensions ( Class_Base_Extension::EXTENSION_NAME_SHMOP ) ) {
            if(!is_cli()){
                $_key   = Class_Base_Auth::SHM_WEB_KEY;
            ...
            if ( ! empty( $_shmid ) ) {
                ...
                if ( ! empty( $_share_memory_security_token ) ) {
                    $_share_memory_security_token = Class_Base_Format::data_to_string ( $_share_memory_security_token );
                    if ( ! empty( $_share_memory_security_token ) ) {
                        if ( ( md5 ( $security_token . Class_Base_Auth::SECURITY_CODE ) ) == $_share_memory_security_token ) {
                            return true;
                        }
                    }
                }
            ...
            if ( ( md5 ( $security_token . Class_Base_Auth::SECURITY_CODE ) ) == PRIVILEGE_USER_MODULE_PASSWORD ) {
                return true;
            }
        }
        return false;
    ...
    public static function password_to_security_token ( $password )
    {
    ...
    public static function reset_password ( $password = null )
    {
        if ( Class_Base_Extension::exist_enabled_extensions ( Class_Base_Extension::EXTENSION_NAME_SHMOP ) ) {
            ...
            if ( ! empty( $_shmid ) ) {
                ...
                $_security_token              = md5 ( $_token . $_security_code );
                $_share_memory_security_token = md5 ( $_security_token . $_security_code );
                $_write_length                = Class_Base_Memory::write_share_memory ( $_shmid , $_share_memory_security_token , Class_Base_Memory::SHARE_MEMORY_OFFSET_START , Class_Base_Memory::DATA_FORMAT_TYPE_STRING_NULL_FILL_PACK );
                if ( empty( $_write_length ) ) {
                    Class_Base_Memory::delete_share_memory_by_key ( $_key , Class_Base_Memory::BLOCK_SIZE_32 );
    ...
    public static function check_privilege_user_and_password_for_cli_create_token ( $privilege_user , $privilege_password )
    {
        if ( ( ! is_string ( $privilege_user ) ) || ( strlen ( $privilege_user ) <= 0 ) ) {
            return false;
        }
    ...
    public static function check_privilege_user_and_password ( $privilege_user , $privilege_password , &$user = null , &$password = null , &$security_token = null )
    {
        if ( is_cli () ) {
            global $_SESSION;
        ...
        if ( md5 ( ( md5 ( ( $privilege_user . Class_Base_Auth::SECURITY_CODE ) ) . Class_Base_Auth::SECURITY_CODE ) ) == PRIVILEGE_USER_MODULE_USER ) {
            if ( md5 ( ( md5 ( ( $privilege_password . Class_Base_Auth::SECURITY_CODE ) ) . Class_Base_Auth::SECURITY_CODE ) ) == PRIVILEGE_USER_MODULE_PASSWORD ) {
                ...
                if ( ! Class_Base_Extension::exist_enabled_extensions ( Class_Base_Extension::EXTENSION_NAME_SHMOP ) ) {
                    ...
                    $_SESSION[ "PHPSPLOIT_FRAMEWORK_SECURITY_TOKEN" ] = $security_token = md5 ( ( $privilege_password . Class_Base_Auth::SECURITY_CODE ) );
                    return true;
                } else {
                    if ( ! self::exist_token () ) {
                        $_SESSION[ "PHPSPLOIT_FRAMEWORK_USER" ]           = $user = $privilege_user;
                        $_SESSION[ "PHPSPLOIT_FRAMEWORK_PASSWORD" ]       = $password = self::create_password ( $privilege_password );
                        $_SESSION[ "PHPSPLOIT_FRAMEWORK_SECURITY_TOKEN" ] = $security_token = md5 ( ( $password . Class_Base_Auth::SECURITY_CODE ) );
                    }
                    return true;
                }
            }
    ...
    public static function check_user_and_password ( $user , $password , &$security_token = null )
    {
        if ( is_cli () ) {
            global $_SESSION;
            if ( ! is_array ( $_SESSION ) ) {
        ...
        if ( Class_Base_Extension::exist_enabled_extensions ( Class_Base_Extension::EXTENSION_NAME_SHMOP ) ) {
            ...
            if ( ! empty( $_shmid ) ) {
                ...
                if ( ! empty( $_share_memory_security_token ) ) {
                    $_share_memory_security_token = Class_Base_Format::data_to_string ( $_share_memory_security_token );
                    if ( ! empty( $_share_memory_security_token ) ) {
                        if ( md5 ( ( md5 ( ( $password . Class_Base_Auth::SECURITY_CODE ) ) . Class_Base_Auth::SECURITY_CODE ) ) == $_share_memory_security_token ) {
                            if ( ( ! isset( $_SESSION ) ) || ( ! is_array ( $_SESSION ) ) ) {
                                $_SESSION = array ();
                            }
