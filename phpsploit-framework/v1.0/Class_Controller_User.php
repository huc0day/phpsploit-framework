...
class Class_Controller_User extends Class_Controller
{
    ...
    public static function create_production_privilege_user_password ( $params = array () )
    {
        ...
        Class_Base_Auth::check_permission ();
        $_init_user            = str_replace ( "?" , "!" , str_replace ( "&" , "!" , str_replace ( "=" , "!" , Class_Base_Request::form ( "init_user" , Class_Base_Request::TYPE_STRING , "huc0day" ) ) ) );
        ...
        if ( is_cli () ) {
            ...
            Class_Base_Response::outputln (
                array (
                    ...
                    "PHPSPLOIT_FRAMEWORK_SECURITY_TOKEN" => ( empty( $_SESSION[ "PHPSPLOIT_FRAMEWORK_SECURITY_TOKEN" ] ) ? "" : $_SESSION[ "PHPSPLOIT_FRAMEWORK_SECURITY_TOKEN" ] ) ,
                )
            );
        }
        if ( ! is_cli () ) {
            ...
            $_cli_encode_url = Class_Base_Response::get_urlencode ( $_cli_url );
            $_form_top       = '<div style="margin-top:64px;margin-bottom:16px;height: 32px;text-align: center;font-size: 18px;">Display account name, account password, Security Token information</div>';
            ...
            $_form           = array (
                ...
                "inputs"    => array (
                    ...
                    array (
                        ...
                        "name"     => "security_token" ,
                        "value"    => ( empty( $_SESSION[ "PHPSPLOIT_FRAMEWORK_SECURITY_TOKEN" ] ) ? "" : $_SESSION[ "PHPSPLOIT_FRAMEWORK_SECURITY_TOKEN" ] ) ,
                        "disabled" => "disabled" ,
                    ) ,
                ) ,
                "textareas" => array (
